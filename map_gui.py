...
def metal_detector_callback(data):
    ...
    for pos in positions:
        if ':' in pos:
            values = pos.split(':')
            if len(values) == 3:
                x, y, mine_type = values
                try:
                    x, y = ord(x) - 65, int(y)-1  # Convert 'A' to 0, 'B' to 1, ..., and adjust y to 0-based index
                    if mine_type == 'buried':
                        map_data[y][x] = 2
                ...
                rospy.logerr(f"Expected 3 values but got {len(values)} in position: {pos}")
        else:
            rospy.logerr(f"Invalid position format: {pos}")
...
while running and not rospy.is_shutdown():
    ...
    for y, row in enumerate(map_data):
        for x, tile in enumerate(row):
            ...
            # Draw coordinates
            coord_text = font.render(f"{chr(65 + x)},{y + 1}", True, (0, 0, 0))  # Convert index to char
